 ...
 */
export class QRAnalyticsIntegrationService {
   ...
   */
  async trackScanEvent(scanData: ScanEvent): Promise<{ success: boolean; error?: string }> {
    try {
      console.log(`[QR-Analytics Integration] Tracking scan for QR: ${scanData.qrCodeId}`);
      
      const response = await axios.post(`${this.analyticsServiceUrl}/track`, scanData, {
        timeout: 5000,
        headers: {
   ...
   */
  async initializeQRCodeAnalytics(qrData: QRGenerationEvent): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  async getQRAnalytics(qrCodeId: string, userId: string, options?: {
    startDate?: Date;
    endDate?: Date;
   ...
   */
  async getAnalyticsSubscriptionInfo(userId: string): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      console.log(`[QR-Analytics Integration] Getting subscription info for User: ${userId}`);
      
      const response = await axios.get(`${this.analyticsServiceUrl}/subscription-info`, {
        timeout: 5000,
        headers: {
          'Content-Type': 'application/json',
          'user-id': userId
   ...
   */
  async exportQRAnalytics(qrCodeId: string, userId: string, format: 'csv' | 'json', options?: {
    startDate?: Date;
    endDate?: Date;
   ...
   */
  async checkAnalyticsHealth(): Promise<{ success: boolean; status?: string; error?: string }> {
    try {
      const response = await axios.get(`${this.analyticsServiceUrl}/health`, {
        timeout: 3000
      });
...
export const qrAnalyticsIntegration = new QRAnalyticsIntegrationService();